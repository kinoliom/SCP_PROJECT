...
#define TEMP_DELAY soft_timer_s_to_ticks(rand() % 20)
...
static void forward_packet(char packet_sent[], uint16_t packet_length, int sender_rank){
    ...
    memset(packet, 0, PHY_MAX_TX_LENGTH - 4);
    int packet_type = packet_sent[0] - '0';
    snprintf(packet, sizeof(packet), "%d%d%s", packet_type, sender_rank, packet_sent + 2);
    uint16_t length_v2 = 1 + strlen(packet);
    ret = mac_csma_data_send(ADDR_BROADCAST, (uint8_t *) packet, length_v2);
        ...
        const uint8_t *data, uint8_t length, int8_t rssi, uint8_t lqi)
{
    ...
    memset(message, 0, PHY_MAX_TX_LENGTH - 4);
    ...
    if (message[0] == '1' && sender_rank < rank){
    ...
    }else if (message[0] == '2' && rank <= sender_rank){
        if( rank == 0){
