...
static void request_temp_measures(){
    ...
    snprintf(packet, sizeof(packet), toSend);
    ...
    char prefix[] = {2, rank};
    char suffix[] = {node.type_str[0],node.num};
    char str_temp[20];
        ...
        const uint8_t *data, uint8_t length, int8_t rssi, uint8_t lqi)
{
    ...
    if (message[0] == 1){
    ...
    }else if (message[0] == 2 && rank == 0){

      int node_type_str = message[22];
      int node_num = message[23];
      ...
      printf("Receive from node %d - %d (Rank: %d): %f °C", node_type_str, node_num, rcv_rank, rcv_temp);
